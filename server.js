...
app.put("/api/profile", requireAuth, async (req, res) => {
  try {
    ...
    await db.updateUserProfile(req.userId, { name, surname: surname.trim(), phone: phone.trim(), email: email.trim() });
    res.json({ ok: true });
...
app.put("/api/change-password", requireAuth, async (req, res) => {
  try {
    ...
    const verify = crypto.pbkdf2Sync(old_password || "", salt, 100000, 64, "sha512").toString("hex");
    let valid = false;
    try { valid = crypto.timingSafeEqual(Buffer.from(hash, "hex"), Buffer.from(verify, "hex")); } catch {}
...
app.get("/api/history", requireAuth, async (req, res) => {
  try {
    ...
    const rows  = await db.getHistory(req.userId, limit);
